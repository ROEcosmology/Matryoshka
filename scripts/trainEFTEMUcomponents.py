...
parser.add_argument(
    "--new_split",
    help="Use a new train test split? 0 for no, 1 for yes. Default 0.",
    default=0,
)
parser.add_argument(
    "--archP110",
    help="Architecture for P110 emulator. pass as a string i.e. '200 200'. This specifies two hidden layers with 200 nodes each. Default '200 200'.",
    default="200 200",
)
parser.add_argument(
    "--archP112",
    help="Architecture for P112 emulator. Default '200 200'.",
    default="200 200",
)
parser.add_argument(
    "--archPloop0",
    help="Architecture for Ploop0 emulator. Default '400 400'.",
    default="400 400",
)
parser.add_argument(
    "--archPloop2",
    help="Architecture for Ploop2 emulator. Default '400 400'.",
    default="400 400",
)
parser.add_argument(
    "--archPct0",
    help="Architecture for Pct0 emulator. Default '200 200'.",
    default="200 200",
)
parser.add_argument(
    "--archPct2",
    help="Architecture for Pct2 emulator. Default '200 200'.",
    default="200 200",
)
parser.add_argument("--verbose", help="Verbose for tensorflow. Default 0.", default=0)
parser.add_argument(
    "--to_train",
    help="Componenets to train. Pass as a string i.e. 'Ploop Pct'. This will only train the Ploop and Pct components. Default 'P11 Ploop Pct'.",
    default="P11 Ploop Pct",
)
parser.add_argument(
    "--kmax",
    help="Maximum k value to train the component emulators to predict. Default is 0.19",
    default=0.19,
)
...
arch_dict = {
    "P11": [
        [int(i) for i in args.archP110.split(" ")],
        [int(i) for i in args.archP112.split(" ")],
    ],
    "Ploop": [
        [int(i) for i in args.archPloop0.split(" ")],
        [int(i) for i in args.archPloop2.split(" ")],
    ],
    "Pct": [
        [int(i) for i in args.archPct0.split(" ")],
        [int(i) for i in args.archPct2.split(" ")],
    ],
}
...
for component in args.to_train.split(" "):
    ...
    P0_nonzero_cols = np.all(P0_data != 0.0, axis=0)
    P2_nonzero_cols = np.all(P2_data != 0.0, axis=0)
    ...
    np.save(
        cache_path + "scalers/{a}0/nonzero_cols".format(a=component), P0_nonzero_cols
    )
    ...
    np.save(
        cache_path + "scalers/{a}2/nonzero_cols".format(a=component), P2_nonzero_cols
    )
    print("Done.")
    ...
    np.save(
        cache_path + "scalers/{a}0/yscaler_min_diff".format(a=component),
        np.vstack([P0_scaler.min_val, P0_scaler.diff]),
    )
    np.save(
        cache_path + "scalers/{a}2/yscaler_min_diff".format(a=component),
        np.vstack([P2_scaler.min_val, P2_scaler.diff]),
    )
    print("Done.")

    reduce_lr = ReduceLROnPlateau(
        monitor="loss", factor=0.1, patience=10, min_lr=0.0001, mode="min", cooldown=10
    )
    ...
    P0_model = MatTrain.trainNN(
        trainx,
        P0_trainy,
        validation_data=None,
        nodes=np.array(arch_dict[component][0]),
        learning_rate=0.013,
        batch_size=100,
        epochs=10000,
    ...
    print(
        "{a}{b} final loss: ".format(a=component, b=0),
        P0_model.evaluate(trainx, P0_trainy),
    )
    ...
    P2_model = MatTrain.trainNN(
        trainx,
        P2_trainy,
        validation_data=None,
        nodes=np.array(arch_dict[component][1]),
        learning_rate=0.013,
        batch_size=100,
        epochs=10000,
        callbacks=callbacks_list,
    ...
    print(
        "{a}{b} final loss: ".format(a=component, b=2),
        P2_model.evaluate(trainx, P2_trainy),
    )

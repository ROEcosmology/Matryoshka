...
parser.add_argument(
    "--inputX",
    help="Directroy with files containg the training cosmologies.",
    required=True,
)
parser.add_argument("--save_dir", help="Path to save outputs.", required=True)
parser.add_argument(
    "--redshift", help="Redshift at which to generate the data.", required=True
)
parser.add_argument(
    "--optiresum",
    help="Boolean. Use pybird optimal resummation. Can be 1 or 0.",
    required=False,
    default=0,
)
...
# Class setup
M = Class()
M.set({"output": "mPk", "P_k_max_1/Mpc": 1.0, "z_max_pk": redshift})
...
np.save(
    save_dir
    + "Ploop0/Ploop0_array-z{z}_optiresum-{o}.npy".format(z=redshift, o=optiresum),
    Ploop0_array,
)
np.save(
    save_dir
    + "Ploop2/Ploop2_array-z{z}_optiresum-{o}.npy".format(z=redshift, o=optiresum),
    Ploop2_array,
)
np.save(
    save_dir + "P110/P110_array-z{z}_optiresum-{o}.npy".format(z=redshift, o=optiresum),
    P110_array,
)
np.save(
    save_dir + "P112/P112_array-z{z}_optiresum-{o}.npy".format(z=redshift, o=optiresum),
    P112_array,
)
np.save(
    save_dir + "Pct0/Pct0_array-z{z}_optiresum-{o}.npy".format(z=redshift, o=optiresum),
    Pct0_array,
)
np.save(
    save_dir + "Pct2/Pct2_array-z{z}_optiresum-{o}.npy".format(z=redshift, o=optiresum),
    Pct2_array,
)

...
cache_path = (
    os.fsdecode(pathlib.Path(os.path.dirname(__file__)).parent.absolute())
    + "/matryoshka-data/"
)
...
relevant_transfer = {"class_aemulus": [0, 1, 3, 5, 6], "QUIP": [0, 1, 2]}
...
class Transfer:
    ...
    def __init__(self, version="class_aemulus"):
        ...
        for member in os.listdir(models_path):
            model = load_model(models_path + member, compile=False)
            models.append(model)
    ...
    def emu_predict(self, X, mean_or_full="mean"):
        ...
        if mean_or_full == "mean":
            ...
            for i in range(len(self.models)):
                preds += self.scalers[1].inverse_transform(self.models[i](X_prime))

            return preds / float(len(self.models))

        elif mean_or_full == "full":

            preds = np.zeros((len(self.models), X_prime.shape[0], self.kbins.shape[0]))
            for i in range(len(self.models)):
...
class Sigma:
    ...
    def emu_predict(self, X, mean_or_full="mean"):
        ...
        if mean_or_full == "mean":
            ...
            preds = np.zeros((len(self.models), X_prime.shape[0], self.mbins.shape[0]))
            for i in range(len(self.models)):
                preds[i, :, :] = self.scalers[1].inverse_transform(
                    self.models[i](X_prime)
                )
...
class Growth:
    ...
    def emu_predict(self, X, mean_or_full="mean"):
        ...
        if mean_or_full == "mean":
            ...
            preds = 0
            for i in range(len(self.models)):
                pred = self.scalers[1].inverse_transform(self.models[i](X_prime))
                pred[:, 0] = 1.0
                preds += pred
            ...
            preds = np.zeros((len(self.models), X_prime.shape[0], self.zbins.shape[0]))
            for i in range(len(self.models)):
                preds[i, :, :] = self.scalers[1].inverse_transform(
                    self.models[i](X_prime)
                )
...
class Boost:
    ...
    def __init__(self, redshift_id):
        ...
        ksim = (ksim[:-1] + ksim[1:]) / 2.0

        self.kbins = ksim
        """The k-bins at which predictions will be made."""

        boost_path = cache_path + "class_aemulus/boost_kwanspace_z{a}/".format(
            a=galaxy_boost_zlist[redshift_id]
        )

        # Load the ensemble of NNs that makes up the B(k) emulator.
        models = list()
        for member in os.listdir(boost_path + "model"):
            model = load_model(boost_path + "model/" + member, compile=False)
...
class MatterBoost:
    ...
    def __init__(self, redshift_id):
        ...
        k, _ = np.loadtxt(cache_path + "QUIP/Pk_m_z=0.txt", unpack=True)
        ks_good = k < 1.0
        ...
        for member in os.listdir(
            models_path + "boost_z{a}".format(a=matter_boost_zlist[redshift_id])
        ):
            model = load_model(
                models_path
                + "boost_z{a}/".format(a=matter_boost_zlist[redshift_id])
                + member,
                compile=False,
            )
        ...
        xmin_diff = np.load(
            scalers_path
            + "boost_z{a}/xscaler_min_diff.npy".format(
                a=matter_boost_zlist[redshift_id]
            )
        )
        ymin_diff = np.load(
            scalers_path
            + "boost_z{a}/yscaler_min_diff.npy".format(
                a=matter_boost_zlist[redshift_id]
            )
        )
...
class Ploopl:
    ...
    def __init__(
        ...
        if version == "custom":
            ...
            xscalers_path = f"{cache_path}{version}/z{redshift}/scalers/"
            yscalers_path = (
                f"{cache_path}{version}/z{redshift}/scalers/Ploop{multipole}/"
            )
    ...
    def emu_predict(self, X):
        ...
        if self.version == "EFTv4":
            ...
            return (
                preds_incl_zeros.reshape(X.shape[0], 12, self.kbins.shape[0])
...
class EFT:
    ...
    def __init__(
        ...
        if version == "custom" and (path_to_model is None):
            raise TypeError(
                "when using version='custom' path_to_model must be specified."
            )

        if version == "custom" and (not os.path.isfile(f"{path_to_model}kbins.npy")):
            raise TypeError(
                "when using version='custom' the directory at path_to_model must contaain a .npy file with the kbins the model predicts the kernels."
            )
    ...
    def emu_predict(self, X, bias, stochastic=None, km=None, ng=None, kvals=None):
        """
        ...
        multipole_array = eft_funcs.multipole_vec(
            [P11_preds, Ploop_preds, Pct_preds], bias, f.reshape(-1, 1)
        )
        ...
        if kvals is not None:
            ...
            if (
                kvals.max() <= self.P11.kbins.max()
                and kvals.min() >= self.P11.kbins.min()
            ):
                multipole_array = interp1d(self.P11.kbins, multipole_array, axis=-1)(
                    kvals
                )
            else:
...
class QUIP:
    ...
    def emu_predict(self, X, kvals=None, mean_or_full="mean"):
        ...
        linPk0 = halo_model_funcs.power0_v2(
            self.Transfer.kbins,
            transfer_preds,
        ...
        growths = halo_model_funcs.DgN_vec(
            X[:, parameter_ids["QUIP"]["Om"]], self.MatterBoost.redshift
        )
        growths /= halo_model_funcs.DgN_vec(X[:, parameter_ids["QUIP"]["Om"]], 0.0)

        linPk = interp1d(self.Transfer.kbins, linPk0, kind="cubic")(
            self.MatterBoost.kbins
        ) * (growths**2).reshape(-1, 1)

        if kvals is not None:
            if (
                kvals.max() < self.MatterBoost.kbins.max()
                and kvals.min() > self.MatterBoost.kbins.min()
            ):
...
class HaloModel:
    ...
    def emu_predict(self, X_COSMO, X_HOD, kspace_filt=False, RT=3.0):
        ...
        T_preds = self.Transfer.emu_predict(X_COSMO, mean_or_full="mean")
        sigma_preds = self.sigma.emu_predict(X_COSMO, mean_or_full="mean")
        dlns_preds = self.dlns.emu_predict(X_COSMO, mean_or_full="mean")
        if self.version == "class_aemulus":
            gf_preds = self.growth.emu_predict(X_COSMO, mean_or_full="mean")

        if self.nonlinear and self.matter:
            boost_preds = self.boost.emu_predict(X_COSMO, mean_or_full="mean")
            # Force the nonlinear boost to unity outside the emulation range.
            boost_preds = interp1d(
                self.boost.kbins, boost_preds, bounds_error=False, fill_value=1.0
            )(self.k)

        elif self.nonlinear:
            boost_preds = self.boost.emu_predict(
                np.hstack([X_HOD, X_COSMO]), mean_or_full="mean"
            )
            # Force the nonlinear boost to unity outside the emulation range.
            boost_preds = interp1d(
                self.boost.kbins, boost_preds, bounds_error=False, fill_value=1.0
        ...
        p_ml = halo_model_funcs.power0_v2(
            self.Transfer.kbins,
            T_preds,
            sigma8=X_COSMO[:, parameter_ids[self.version]["sigma8"]],
        ...
        if self.version == "class_aemulus":
            ...
            for i in range(D_z.shape[0]):
                ...
                D_z[i] = halo_model_funcs.DgN(
                    X_COSMO[i, 0], self.redshift
                ) / halo_model_funcs.DgN(X_COSMO[i, 0], 0.0)
        ...
        for i in range(X_HOD.shape[0]):
            ...
            n_t = halo_model_funcs.ngal(
                self.sigma.mbins[tm].reshape(1, -1),
                hmf.reshape(1, -1),
                Ntot.reshape(1, -1),
            )[0]
            n_ts[i] = n_t

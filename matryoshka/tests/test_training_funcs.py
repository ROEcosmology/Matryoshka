"""Tests for :mod:`matryoshka.training_funcs`."""
import numpy as np
import pytest
...
class TestScalers:
    ...
    def test_UnifromScaler(self):
        ...
        assert np.all(
            scaler.inverse_transform(scaler.transform(self.data))
            == (data_prime * self.diff) + self.min_val
        )
    ...
    def test_LogScaler(self):
        ...
        # Check inverse transform.
        assert np.all(
            scaler.inverse_transform(scaler.transform(np.abs(self.data)))
    ...
    def test_StandardScaler(self):
        ...
        assert np.all(
            scaler.inverse_transform(scaler.transform(self.data))
            == (data_prime * self.stddev) + self.mean
        )

...
def sample_space(
    ...
    filename=None,
    set_labels=None,
    colour_variable=None,
    bounds=None,
    bounds_colour='lightgray',
    param_label_size='large',
    tick_label_size='medium',
    leg_label_size='large',
    marker_sizes=None,
):
    """
    Corner plot of the sample space.

    Adapted from :pkg:`corner` by Daniel Foreman-Mackey.
...
def per_err(
    ...
    plt.fill_between(
        xvalues,
        np.percentile(per_err, 2.5, axis=0),
        np.percentile(per_err, 100 - 2.5, axis=0),
        label=r"$95\%$",
        alpha=0.3,
        color='tab:blue',
    )
    plt.fill_between(
        xvalues,
        np.percentile(per_err, 16, axis=0),
        np.percentile(per_err, 100 - 16, axis=0),
        label=r"$68\%$",
        alpha=0.3,
        color='tab:green',
    )

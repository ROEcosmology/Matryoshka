...
def RSD(
    Pk, kbins, mu_bins, beta, poles=None, fog=True, sigma=None, qperp=None, qpar=None
):
    """
    ...
    if (qperp is not None) and (qpar is not None):
        ...
        Pk = interp1d(
            kbins, Pk, kind="cubic", bounds_error=False, fill_value="extrapolate"
        )(kp)
...
def AP(lPk, mu_bins, kbins, qperp, qpar):
    ...
    Pkint = interp1d(
        kbins, lPk, kind="cubic", bounds_error=False, fill_value="extrapolate", axis=-1
    )(kp)
    # print(Pkint.shape)
    ...
    for pole in range(2):
        ...
        Pell.append(
            (2 * l + 1) / (2 * qperp**2 * qpar) * trapz(Interg, x=mugrid, axis=-1)
        )
    return np.stack(Pell)
...
def DA_vec(Om, z):
    ...
    r = trapz(1.0 / Hubble(Om_grid, z_grid), x=z_eval, axis=0)
    return r / (1 + z)

...
def _p(K, bs, bc, asi, ac, c):
    ...
    """
    return (
        np.sin(K) * (asi - bs) - np.sin(c * K) / ((1 + c) * K) + np.cos(K) * (ac - bc)
    )
...
def sat_Z09(M, logM1, alpha, kappa, logM_cut):
    ...
    mean[idx_nonzero] = ((M[idx_nonzero] - kappa * 10**logM_cut) / (10**logM1)) ** alpha
    return mean
...
def mean_density0_v2(h, Om0):
    ...
    critdens_const = 3.0 / (8.0 * np.pi * consts.G * 1000)
...
def halomodel_power(
    ...
    mean_dens = mean_density0_v2(h=cosmo.H0 / 100.0, Om0=cosmo.Om0)
    hmf_matry = hmf(sigma, dlns, mean_dens, m, growth, redshift)

    u_m = u(k, m[tm], conc[tm], mean_dens, 200)

    n_t = ngal(
        m[tm].reshape(1, -1), hmf_matry[tm].reshape(1, -1), (N_tot).reshape(1, -1)
    )[0]
    pm_matry = power0_v2(k, transfer, sigma8=sigma8, ns=ns)[0]
    if nonlinear:
        pm_matry = nonlinear_power(k, pm_matry * growth**2, sigma8, redshift, cosmo)
    else:
    ...
    P2h = power_2h(u_m, hmf_matry[tm], m[tm], N_tot, n_t, pm_matry, halo_bias[tm])
    if split_1h_2h:
...
def Gcov(P_k, k, dk, n, V):
    ...
    """
    N_k = (4 * np.pi * k**2 * dk) / ((2 * np.pi) / V ** (1.0 / 3.0)) ** 3
...
def fN(Om, z):
    ...
    a = 1.0 / (1.0 + z)
    return (Om * (5 * a - 3 * DgN(Om, z))) / (2.0 * (a**3 * (1 - Om) + Om) * DgN(Om, z))
...
def DgN_vec(Om, z):
    ...
    return (
        ...
        * cH(Om, a)
        / a
        * trapz(cH(Om_grid, a_grid) ** -3, x=a_eval, axis=0)
    )


def fN_vec(Om, z):
    a = 1.0 / (1.0 + z)
    return (Om * (5 * a - 3 * DgN_vec(Om, z))) / (
        2.0 * (a**3 * (1 - Om) + Om) * DgN_vec(Om, z)
    )
